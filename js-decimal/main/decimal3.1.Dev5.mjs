...
class decimal{
    ...
    [Symbol.toStringTag]="decimal3.1"
}

//UMD形式ってよく分かんないからAI産
function export_(libraryData, globalName) {
    if (typeof define === 'function' && define.amd) {
        // AMD環境
        define([], function() { return libraryData; });
    } else if (typeof module === 'object' && module.exports) {
        // CommonJS環境 (Node.js)
        module.exports = libraryData;
    } else {
        // 【ここがポイント】
        // 通常の<script>タグ（非モジュール）の時だけ globalThis に登録する
        // ※ ESMとして読み込まれている（thisが非存在）なら、globalThis を汚さない
        if (typeof this !== 'undefined') {
            globalThis[globalName] = libraryData;
        }
    }
}
export_({decimalsup,decimalPack,decimal,decimalAdmin},"decimalAllPackage")
