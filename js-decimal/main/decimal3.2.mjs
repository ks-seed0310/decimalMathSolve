...
class decimalPack{
    ...
    constructor(value,limit=Infinity,constshift=null,expmode=false){
        if (expmode||String(value).includes("e")){
            this.#expSetup(String(value),limit,constshift)
        }else{
...
class decimalAdmin{
    #target
    //mode=0: decimal instance
    //mode=1: extends decimal instance
    #mode
    constructor(target_Decimal){
        if ((target_Decimal instanceof decimal)){
            if (target_Decimal.isDeveloperMode){
                this.#target=target_Decimal
                if (target_Decimal.constructor===decimal){
                    this.#mode=0
                }else{
                    this.#mode=1
                }
            ...
            throw new TypeError("can input type is decimal class or extends decimal instance.")
        }
    }
    get returnQuantize(){
...
class decimal{
    ...
    developerModeFunctionsGate(cmd,...args){
        ...
        if (cmd===0){
            //Config Reset
            this.#config=structuredClone(this.#config_template);
            this._returnClassFunctionUpdate()
        }
        if (cmd===1){
            //Config Setting
    ...
    #mod_backend_old(...values){//数学のモジュロ演算
        //mod(1,2,3)-->(1%2)%3
        if (values.length===0)return 0
        if (values.length===1)return values[0]
    ...
    #mod_backend(...values){//JavaScriptのモジュロ演算
        if (values.length===0)return 0
        if (values.length===1)return values[0]
    ...
    #sqrt_ts(a){
        const val={
            $0_1__9_9$1:this.#sqrtConstant["$"+(Math.ceil(10*a)/10).toString().replace(".","_")],
            $0_1__9_9$2:this.#sqrtConstant["$"+(Math.floor(10*a)/10).toString().replace(".","_")],
            $10__100$1:this.#sqrtConstant["$"+(Math.ceil(a/2)*2).toString()],
            $10__100$2:this.#sqrtConstant["$"+(Math.floor(a/2)*2).toString()],
        }
        return val.$0_1__9_9$1??(val.$0_1__9_9$2??(val.$10__100$1??(val.$10__100$2??null)))
    }
    ...
    [Symbol.toStringTag]="decimal3.2"
    #apiConnectObject=null
    #apiConnecting_=false
    #apiConnectKey=Symbol("decimal3.2_API_Connection_Key")
    get isApiConnecting(){
        return this.#apiConnecting_
    }
    ____api_systemConnectGate_submit(instance){
        if (this.#apiConnectObject!==null){
            return 2
        }
        if (instance instanceof decimal){
            if (instance.constructor!==decimal){
                if (instance.isDeveloperMode){
                    if (instance.____system_decimalConnect===true&&instance.____system_decimalConnect_func(this)==="decimal3.2 - decimal for JavaScript Scripts Backend Function API Connecting System Connect Keys"){
                        this.#apiConnectObject=instance
                        this.#apiConnecting_=true
                        return this.#apiConnectKey
                    }else{
                        return 1
                    }
                }else{
    ...
    _api(instance,key,command,...args){
        if (this.#apiConnectObject!==null&&instance===this.#apiConnectObject&&key===this.#apiConnectKey){
            switch(command){
                case "add":
                    return this.#add_backend(...args)
                case "sub":
                    return this.#sub_backend(...args)
                case "mul":
                    return this.#mul_backend(...args)
                case "mul_bigint":
                    return this.#mul_bigint_calc(...args)
                case "div":
                    return this.#div_backend(...args)
                case "mod_math":
                    return this.#mod_backend_old(...args)
                case "mod_lang":
                    return this.#mod_backend(...args)
                case "pow":
                    return this.#pow_backend(...args)
                case "pow_int_int":
                    return this.#xInt_pow(...args)
                case "pow_float_int":
                    return this.#xFloat_pow(...args)
                case "pow_any_float":
                    return this.#yFloat_pow(...args)
                case "ln_o":
                    return this.#ln_o(...args)
                case "log_ts":
                    return this.#log_ts(...args)
                case "log":
                    return this.#log_backend(...args)
                case "exp":
                    return this.#exp_backend(...args)
                case "sin":
                    return this.#sin_backend(...args)
                case "cos":
                    return this.#cos_backend(...args)
                case "tan":
                    return this.#tan_backend(...args)
                case "sqrt_ts":
                    return this.#sqrt_ts(...args)
                case "sqrt":
                    return this.#sqrt_backend(...args)
                case "sinh":
                    return this.#sinh_backend(...args)
                case "cosh":
                    return this.#cosh_backend(...args)
                case "tanh":
                    return this.#tanh_backend(...args)
                case "arsinh":
                    return this.#arsinh_backend(...args)
                case "arcosh":
                    return this.#arcosh_backend(...args)
                case "artanh":
                    return this.#artanh_backend(...args)
                case "arcsin":
                    return this.#arcsin_backend(...args)
                case "arccos":
                    return this.#arccos_backend(...args)
                case "arctan":
                    return this.#arctan_backend(...args)
                case "return":
                    return this.#rp(...args)
                case "get_config":
...
//UMD形式ってよく分かんない
function export_(libraryDataObj, globalNamesArray) {
    if (typeof define === 'function' && define.amd) {
        // AMD環境
        define([], function() { return libraryDataObj; });
    } else if (typeof module === 'object' && module.exports) {
        // CommonJS環境 (Node.js)
        module.exports = libraryDataObj;
    } else {
        // ブラウザグローバル環境 (通常の <script> タグ)
        if (Array.isArray(globalNamesArray)) {
            const values = Object.values(libraryDataObj);
            globalNamesArray.forEach((originalName, index) => {
                const targetValue = values[index];
                if (targetValue !== undefined) {
                    globalThis[originalName] = targetValue;
                }
            });
        }
    }
...
//import構文がおすすめです
export_({decimalsup,decimalPack,decimal,decimalAdmin},["_decimalsup","_decimalPack","decimal","decimalAdmin"])
