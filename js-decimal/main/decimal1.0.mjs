...
class decimalMath{
    ...
    #returns=String
    #returns_isNew=false
    #mulConfig={karatubaThreshold:2n**2048n}
    #globalConfig={mul_karatubaExecute:true}
    #globalConfig_needprop=["mul_karatubaExecute"]
    setglobalConfig(value){
        const k=Object.keys(value)
        const okt=this.#globalConfig_needprop.map(x=>k.includes(x))
        if (okt.includes(false))throw(new(TypeError(`The Config data is missing a required property. The required property is "${this.#globalConfig_needprop.join(" , ")}".`)))
    }
    setprecision(value){
        if(typeof(value)!=="number")throw(new(TypeError("Only the Number data type can be used for precision.")));
    ...
    #int_mul(...values){
        ...
        if (is.includes(false))throw new TypeError("Only Number, BigInt, and String arguments are accepted.")
        ...
        const n=val[0]
        const m=val[1]
        if (!this.#globalConfig.mul_karatubaExecute){
            return n*m
        }
        const nl=n.toString(2).length
        const ml=m.toString(2).length
        const kk=this.#mulConfig.karatubaThreshold
        if (nl<kk||ml<kk) {
            return n*m
        }
    ...
    pow(n,Exp=2){
        if (!Number.isInteger(Exp))throw new RangeError('Decimal exponentiation is not currently supported. Please use "decimal_addon_number.js", which will probably be released in the future.')
        let exp=BigInt(Exp)
